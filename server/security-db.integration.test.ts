...
describe.skipIf(!run)('security database concurrency', () => {
  ...
  it('atomically permits only one transaction authorization consumer', async () => {
    ...
    await database!.from('profiles').insert([
      {id: senderId, solana_wallet_address: `sender-${senderId}`, phone_number: phone},
      {id: recipientId, solana_wallet_address: `recipient-${recipientId}`, phone_number: '+2348098765432'},
    ]).throwOnError();
    ...
    await database!.from('transaction_authorizations').insert({
      ...
      recipient_profile_id: recipientId, amount_lamports: 10,
    ...
    try {
      const consume = () => database!.rpc('consume_transaction_authorization', {
        ...
        p_recipient_profile_id: recipientId, p_amount_lamports: 10,
      });
